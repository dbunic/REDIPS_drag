...
Apr 06, 2011.
*/
"use strict";var REDIPS=REDIPS||{};REDIPS.drag=(function(){var init,init_tables,enable_drag,img_onmousemove,handler_onmousedown,table_top,handler_onmouseup,handler_onmousemove,cell_changed,handler_onresize,set_trc,set_position,set_bgcolor,get_bgcolor,box_offset,calculate_cells,getScrollPosition,autoscrollX,autoscrollY,clone_object,clone_limit,elementControl,trash_delete,get_style,save_content,move_object,row_opacity,row_clone,row_drop,obj_margin=null,window_width=0,window_height=0,scroll_width=null,scroll_height=null,edge={page:{x:0,y:0},div:{x:0,y:0},flag:{x:0,y:0}},scroll_object,bgcolor_old,scrollable_container=[],tables=[],moved_flag=0,cloned_flag=0,cloned_id=[],currentCell=[],div_drag=null,div_box=null,pointer={x:0,y:0},top_position,table=null,table_old=null,table_source=null,row=null,row_old=null,row_source=null,cell=null,cell_old=null,cell_source=null,obj=false,obj_old=false,mode='cell',hover_color='#E7AB83',bound=25,speed=20,only={div:[],cname:'only',other:'deny'},mark={action:'deny',cname:'mark',exception:[]},border='solid',border_disabled='dotted',trash='trash',trash_ask=true,drop_option='multiple',delete_cloned=true,source_cell=null,current_cell=null,previous_cell=null,target_cell=null,clone_shiftKey=false;init=function(dd){var self=this,i,imgs,obj_new_div;if(dd===undefined){dd='drag';}
...
REDIPS.event.add(window,'scroll',calculate_cells);if(!Array.prototype.indexOf){Array.prototype.indexOf=function(el){var i;for(i=0;i<this.length;i++){if(this[i]===el){return i;}}
return-1;};}};init_tables=function(){var i,j,element,table_type,tables_nodeList,contain_nested,td,rowspan;top_position=0;tables_nodeList=div_drag.getElementsByTagName('table');for(i=0;i<tables_nodeList.length;i++){element=tables_nodeList[i].parentNode;do{if(element===div_drag){contain_nested=tables_nodeList[i].getElementsByTagName('table');if(contain_nested.length===0){table_type=2;top_position++;}
else{table_type=0;}
break;}
if(element.nodeName==='TD'){table_type=1;element.table=true;top_position++;break;}
element=element.parentNode;}while(element);tables[i]=tables_nodeList[i];tables[i].redips_tableType=table_type;tables[i].redips_idx=i;td=tables[i].getElementsByTagName('td');for(j=0,rowspan=false;j<td.length;j++){if(td[j].rowSpan>1){rowspan=true;break;}}
tables[i].redips_rowspan=rowspan;}
tables.sort(function(a,b){return b.redips_tableType-a.redips_tableType;});};img_onmousemove=function(){return false;};handler_onmousedown=function(e){var evt=e||window.event,offset,mouseButton,position,X,Y;X=pointer.x=evt.clientX;Y=pointer.y=evt.clientY;if(elementControl(evt)){return true;}
...
else{REDIPS.drag.mode=mode='row';REDIPS.drag.obj=obj=row_clone(obj,false);}
...
idx=tables.indexOf(e);if(e.redips_tableType>0&&idx!==0){tmp=tables[0];tables[0]=tables[idx];tables[idx]=tmp;}
else if(e.redips_tableType===0&&idx!==top_position){tmp=tables[top_position];tables[top_position]=tables[idx];tables[idx]=tmp;}};row_clone=function(el){var table_mini,offset,row_obj,row_index,div1,div2,row_last,i;if(el.tagName==='TR'){row_obj=el;while(el&&el.nodeName!=='TABLE'){el=el.parentNode;}
row_index=row_obj.rowIndex;table_mini=el.cloneNode(true);row_last=table_mini.rows.length-1;for(i=row_last;i>=0;i--){if(i!==row_index){table_mini.deleteRow(i);}}
div1=row_obj.getElementsByTagName('div');div2=table_mini.getElementsByTagName('div');for(i=0;i<div2.length;i++){div2[i].redips_enabled=div1[i].redips_enabled;div2[i].redips_container=div1[i].redips_container;if(div1[i].redips_enabled){div2[i].onmousedown=handler_onmousedown;}}
table_mini.redips_container=div_drag;document.getElementById('obj_new').appendChild(table_mini);offset=box_offset(row_obj,'fixed');table_mini.style.position='fixed';table_mini.style.top=offset[0]+"px";table_mini.style.left=offset[3]+"px";table_mini.style.width=(offset[1]-offset[3])+"px";return table_mini;}
else{while(el&&el.nodeName!=='TR'){el=el.parentNode;}
return el;}};row_drop=function(r_table,r_row){var tr;tables[table_source].deleteRow(row_source);tr=obj.getElementsByTagName('tr')[0];tables[r_table].rows[r_row].parentNode.insertBefore(tr,tables[r_table].rows[r_row]);obj.parentNode.removeChild(obj);};handler_onmouseup=function(e){var evt=e||window.event,target_table,r_table,r_row,mt_tr,X,Y,i,target_elements,target_elements_length;X=evt.clientX;Y=evt.clientY;edge.flag.x=edge.flag.y=0;if(obj.releaseCapture){obj.releaseCapture();}
...
else if(moved_flag===0){REDIPS.drag.myhandler_notmoved();target_cell.appendChild(obj);}
...
table_old=row_old=cell_old=null;};handler_onmousemove=function(e){var evt=e||window.event,bound=REDIPS.drag.bound,sca,X,Y,i,scrollPosition;X=pointer.x=evt.clientX;Y=pointer.y=evt.clientY;if(moved_flag===0&&(obj.className.indexOf('clone')>-1||(REDIPS.drag.clone_shiftKey===true&&evt.shiftKey))){clone_object();cloned_flag=1;REDIPS.drag.myhandler_cloned();set_position();}
else if(moved_flag===0){if(mode==='row'){REDIPS.drag.obj_old=obj_old=obj;REDIPS.drag.obj=obj=row_clone(obj);obj.style.zIndex=999;}
if(obj.setCapture){obj.setCapture();}
...
if(cell_current.className.indexOf(REDIPS.drag.trash)===-1){only_found=cell_current.className.indexOf(REDIPS.drag.only.cname)>-1?true:false;if(only_found===true){if(cell_current.className.indexOf(only.div[obj.id])===-1){if((table_old!==null&&row_old!==null&&cell_old!==null)){table=table_old;row=row_old;cell=cell_old;}
break;}}
...
else{REDIPS.event.add(scroll_object,'scroll',calculate_cells);edge.flag.y=0;currentCell=[0,0,0,0];}};clone_object=function(){var obj_new=obj.cloneNode(true),offset,offset_dragged;document.getElementById('obj_new').appendChild(obj_new);if(obj_new.setCapture){obj_new.setCapture();}
...
classes=classes.replace(/^\s+|\s+$/g,'').replace(/\s{2,}/g,' ');obj_old.className=classes;}};elementControl=function(evt){var flag=false,srcName,classes,regex_nodrag=/\bnodrag\b/i;if(evt.srcElement){srcName=evt.srcElement.tagName;classes=evt.srcElement.className;}
else{srcName=evt.target.tagName;classes=evt.target.className;}
...
return flag;};trash_delete=function(){var div_text='element',border;if(obj.className.indexOf('t1')>0){border='green';}
...
REDIPS.drag.myhandler_undeleted();}};enable_drag=function(enable_flag,div_id){var i,j,k,divs=[],tbls=[],borderStyle,cursor,overflow,autoscroll,enabled,cb,handler,position,regex_drag=/\bdrag\b/i,regex_noautoscroll=/\bnoautoscroll\b/i;if(enable_flag===true||enable_flag==='init'){handler=handler_onmousedown;borderStyle=REDIPS.drag.border;cursor='move';enabled=true;}
...
for(i=0,j=0;i<divs.length;i++){if(regex_drag.test(divs[i].className)){divs[i].onmousedown=handler;divs[i].style.borderStyle=borderStyle;divs[i].style.cursor=cursor;divs[i].redips_enabled=enabled;divs[i].redips_container=div_drag;}
...
j++;}}}};get_style=function(el,style_name){var val;if(el.currentStyle){val=el.currentStyle[style_name];}
