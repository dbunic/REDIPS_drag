...
element=element.parentNode;}while(element&&element!==div_drag);tables[i]=tables_nodeList[i];tables[i].redips_container=div_drag;tables[i].redips_nestedLevel=level;tables[i].redips_idx=i;td=tables[i].getElementsByTagName('td');for(j=0,rowspan=false;j<td.length;j++){if(td[j].rowSpan>1){rowspan=true;break;}}
