...
tables.sort(function(a,b){return b.redips.sort-a.redips.sort;});sort_idx++;};row_clone=function(el){var table_mini,offset,row_obj,row_index,row_last,id,i;if(el.nodeName==='TR'){row_obj=el;el=find_parent('TABLE',el);row_index=row_obj.rowIndex;table_mini=el.cloneNode(true);table_mini.redips={};table_mini.redips.container=el.redips.container;table_mini.redips.source_row=row_obj;row_last=table_mini.rows.length-1;for(i=row_last;i>=0;i--){if(i!==row_index){table_mini.deleteRow(i);}}
form_elements(row_obj,table_mini.rows[0]);copy_properties(row_obj,table_mini);if(row_obj.redips!==undefined){table_mini.redips.dragrow_id=row_obj.redips.dragrow_id;}
document.getElementById('redips_clone').appendChild(table_mini);offset=box_offset(row_obj,'fixed');table_mini.style.position='fixed';table_mini.style.top=offset[0]+"px";table_mini.style.left=offset[3]+"px";table_mini.style.width=(offset[1]-offset[3])+"px";return table_mini;}
else{id=el.id;el=find_parent('TR',el);el.redips={};el.redips.dragrow_id=id;return el;}};row_drop=function(r_table,r_row,table_mini){var ts=tables[r_table].rows[0].parentNode,tr,src,rowIndex;if(table_mini===undefined){table_mini=obj;}
src=table_mini.redips.source_row;rowIndex=src.rowIndex;src=find_parent('TABLE',src);src.deleteRow(rowIndex);tr=table_mini.getElementsByTagName('tr')[0];if(r_row<tables[r_table].rows.length){ts.insertBefore(tr,tables[r_table].rows[r_row]);}
...
while(tables[table].redips.rowspan&&cell===-1&&row-->0);if(row<0||cell<0){table=table_old;row=row_old;cell=cell_old;}
...
div_cloned.id=div.id+'c'+cloned_id[div.id];cloned_id[div.id]+=1;copy_properties(div,div_cloned);return(div_cloned);};copy_properties=function(el1,el2){var div1,div2,copy,i;copy=function(d1,d2){if(d1.redips){d2.redips={};d2.redips.enabled=d1.redips.enabled;d2.redips.container=d1.redips.container;if(d1.redips.enabled){d2.onmousedown=handler_onmousedown;d2.ondblclick=handler_ondblclick;}}};if(el1.nodeName==='DIV'){copy(el1,el2);}
div1=el1.getElementsByTagName('div');div2=el2.getElementsByTagName('div');for(i=0;i<div2.length;i++){copy(div1[i],div2[i]);}};clone_limit=function(){var match_arr,limit_type,limit,classes;classes=obj_old.className;match_arr=classes.match(/climit(\d)_(\d+)/);if(match_arr!==null){limit_type=parseInt(match_arr[1],10);limit=parseInt(match_arr[2],10);limit-=1;classes=classes.replace(/climit\d_\d+/g,'');if(limit<=0){classes=classes.replace('clone','');if(limit_type===2){classes=classes.replace('drag','');obj_old.onmousedown=null;obj_old.style.cursor='auto';REDIPS.drag.myhandler_clonedend2();}
...
else if(p.obj.className.indexOf('row')>-1){p.mode='row';p.obj=p.obj_old=find_parent('TR',p.obj);p.obj=row_clone(p.obj_old);}
else{p.mode='cell';}
