...
X=pointer.x=evt.clientX;Y=pointer.y=evt.clientY;REDIPS.drag.obj_old=obj_old=obj||this;REDIPS.drag.obj=obj=this;clone_class=obj.className.indexOf('clone')>-1?true:false;table_top(obj);if(div_drag!==obj.redips.container){div_drag=obj.redips.container;init_tables();}
...
moved=cloned=false;REDIPS.event.add(document,'mousemove',handler_onmousemove);REDIPS.event.add(document,'mouseup',handler_onmouseup);if(obj.setCapture){obj.setCapture();}
...
tables.sort(function(a,b){return b.redips.sort-a.redips.sort;});sort_idx++;};row_clone=function(el){var table_mini,offset,row_obj,last_idx,last_row=true,cr,div,i,j;if(el.nodeName==='DIV'){div=el;el=find_parent('TR',el);if(el.redips===undefined){el.redips={};}
...
last_idx=table_mini.rows.length-1;for(i=last_idx;i>=0;i--){if(i!==row_obj.rowIndex){if(last_row===true){cr=table_mini.rows[i];for(j=0;j<cr.cells.length;j++){if(cr.cells[j].className.indexOf('rowhandler')>-1){last_row=false;break;}}}
table_mini.deleteRow(i);}}
row_obj.redips.last_row=last_row;table_mini.redips={};table_mini.redips.container=el.redips.container;table_mini.redips.source_row=row_obj;form_elements(row_obj,table_mini.rows[0]);copy_properties(row_obj,table_mini.rows[0]);document.getElementById('redips_clone').appendChild(table_mini);offset=box_offset(row_obj,'fixed');table_mini.style.position='fixed';table_mini.style.top=offset[0]+"px";table_mini.style.left=offset[3]+"px";table_mini.style.width=(offset[1]-offset[3])+"px";return table_mini;}};row_drop=function(r_table,r_row,table_mini){var tbl=tables[r_table],ts=tbl.rows[0].parentNode,animated=false,tr,rp,src,rowIndex,delete_srow;delete_srow=function(){if(!animated&&obj_old.redips.last_row){row_opacity(obj_old,'empty','White');}
...
if(r_row<tbl.rows.length){ts.insertBefore(tr,tbl.rows[r_row]);rp=tbl.rows[r_row+1].redips;if(rp&&rp.last_row){ts.deleteRow(r_row+1);}}
else{ts.appendChild(tr);}
delete tr.redips.last_row;if(!animated){REDIPS.drag.myhandler_row_dropped(target_cell);}}
...
obj.parentNode.removeChild(obj);delete obj_old.redips.last_row;if(cloned){REDIPS.drag.myhandler_row_notcloned();}
...
else if(target_cell.className.indexOf(REDIPS.drag.trash_cname)>-1){obj.parentNode.removeChild(obj);if(REDIPS.drag.trash_ask){setTimeout(function(){if(confirm('Are you sure you want to delete?')){REDIPS.drag.myhandler_deleted();if(cloned){clone_limit();}}
...
else{REDIPS.drag.myhandler_deleted();if(cloned){clone_limit();}}}
...
calculate_cells();}
table_old=row_old=cell_old=null;};element_drop=function(){var drop=REDIPS.drag.myhandler_dropped_before(target_cell);if(drop!==false){target_cell.appendChild(obj);REDIPS.drag.myhandler_dropped(target_cell);if(cloned){REDIPS.drag.myhandler_cloned_dropped(target_cell);clone_limit();}}
else if(cloned){obj.parentNode.removeChild(obj);}};handler_onmousemove=function(e){var evt=e||window.event,bound=REDIPS.drag.bound,sca,X,Y,i,scrollPosition;X=pointer.x=evt.clientX;Y=pointer.y=evt.clientY;if(!moved){if(mode==='cell'&&(clone_class||(REDIPS.drag.clone_shiftKey===true&&shift_key))){REDIPS.drag.obj_old=obj_old=obj;REDIPS.drag.obj=obj=clone_div(obj,true);cloned=true;REDIPS.drag.myhandler_cloned();set_position();}
...
return i;};normalize=function(str){return str.replace(/^\s+|\s+$/g,'').replace(/\s{2,}/g,' ');};row_opacity=function(el,opacity,color){var td,i,j;if(typeof(el)==='string'){el=document.getElementById(el);el=find_parent('TABLE',el);}
