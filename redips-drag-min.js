...
threshold.x=X;threshold.y=Y;threshold.flag=false;REDIPS.drag.obj_old=obj_old=obj||this;REDIPS.drag.obj=obj=this;clone_class=obj.className.indexOf('clone')>-1?true:false;if(REDIPS.drag.table_sort){table_top(obj);}
if(div_drag!==obj.redips.container){div_drag=obj.redips.container;init_tables();}
...
REDIPS.event.remove(document,'mousemove',handler_onmousemove);REDIPS.event.remove(document,'touchmove',handler_onmousemove);REDIPS.event.remove(document,'mouseup',handler_onmouseup);REDIPS.event.remove(document,'touchend',handler_onmouseup);div_drag.onselectstart=null;reset_styles(obj);scroll_width=document.documentElement.scrollWidth;scroll_height=document.documentElement.scrollHeight;edge.flag.x=edge.flag.y=0;if(cloned&&mode==='cell'&&(table===null||row===null||cell===null)){obj.parentNode.removeChild(obj);cloned_id[obj_old.id]-=1;REDIPS.drag.myhandler_notcloned();}
...
register_events(obj);REDIPS.drag.myhandler_dropped(target_cell);if(cloned){REDIPS.drag.myhandler_cloned_dropped(target_cell);clone_limit();}}
else if(cloned){obj.parentNode.removeChild(obj);}};register_events=function(div,flag){if(flag===false){div.onmousedown=null;div.ontouchstart=null;div.ondblclick=null;}
else{div.onmousedown=handler_onmousedown;div.ontouchstart=handler_onmousedown;div.ondblclick=handler_ondblclick;}};reset_styles=function(el){el.style.top='';el.style.left='';el.style.position='';el.style.zIndex='';};element_deleted=function(){var param;if(cloned){clone_limit();}
...
REDIPS.drag.myhandler_deleted(cloned);};handler_onmousemove=function(e){var evt=e||window.event,bound=REDIPS.drag.bound,sca,X,Y,deltaX,deltaY,i,scrollPosition;if(evt.touches){X=pointer.x=evt.touches[0].clientX;Y=pointer.y=evt.touches[0].clientY;}
else{X=pointer.x=evt.clientX;Y=pointer.y=evt.clientY;}
...
if(REDIPS.drag.autoscroll){edge.page.x=bound-(window_width/2>X?X-obj_margin[3]:window_width-X-obj_margin[1]);if(edge.page.x>0){if(edge.page.x>bound){edge.page.x=bound;}
...
else{REDIPS.event.add(scroll_object,'scroll',calculate_cells);edge.flag.y=0;currentCell=[0,0,0,0];}};clone_div=function(div,drag){var div_cloned=div.cloneNode(true),cname=div_cloned.className,offset,offset_dragged;if(drag===true){document.getElementById('redips_clone').appendChild(div_cloned);div_cloned.style.zIndex=999;div_cloned.style.position='fixed';offset=box_offset(div);offset_dragged=box_offset(div_cloned);div_cloned.style.top=(offset[0]-offset_dragged[0])+'px';div_cloned.style.left=(offset[3]-offset_dragged[3])+'px';}
if(div_cloned.setCapture){div_cloned.setCapture();}
cname=cname.replace('clone','');cname=cname.replace(/climit(\d)_(\d+)/,'');div_cloned.className=normalize(cname);if(cloned_id[div.id]===undefined){cloned_id[div.id]=0;}
div_cloned.id=div.id+'c'+cloned_id[div.id];cloned_id[div.id]+=1;copy_properties(div,div_cloned);return(div_cloned);};copy_properties=function(src,cln){var copy=[],childs;copy[0]=function(e1,e2){if(e1.redips){e2.redips={};e2.redips.enabled=e1.redips.enabled;e2.redips.container=e1.redips.container;if(e1.redips.enabled){register_events(e2);}}};copy[1]=function(e1,e2){if(e1.redips){e2.redips={};e2.redips.empty_row=e1.redips.empty_row;}};childs=function(e){var el1,el2,i,tn=['DIV','TR'];el1=src.getElementsByTagName(tn[e]);el2=cln.getElementsByTagName(tn[e]);for(i=0;i<el2.length;i++){copy[e](el1[i],el2[i]);}};if(src.nodeName==='DIV'){copy[0](src,cln);}
else if(src.nodeName==='TR'){copy[1](src,cln);}
childs(0);childs(1);};clone_limit=function(){var match_arr,limit_type,limit,classes;classes=obj_old.className;match_arr=classes.match(/climit(\d)_(\d+)/);if(match_arr!==null){limit_type=parseInt(match_arr[1],10);limit=parseInt(match_arr[2],10);limit-=1;classes=classes.replace(/climit\d_\d+/g,'');if(limit<=0){classes=classes.replace('clone','');if(limit_type===2){classes=classes.replace('drag','');register_events(obj_old,false);obj_old.style.cursor='auto';REDIPS.drag.myhandler_clonedend2();}
...
return flag;};enable_drag=function(enable_flag,el,type){var i,j,k,divs=[],tbls=[],borderStyle,opacity,cursor,overflow,autoscroll,enabled,cb,position,regex_drag=/\bdrag\b/i,regex_noautoscroll=/\bnoautoscroll\b/i;opacity=REDIPS.drag.opacity_disabled;if(enable_flag===true||enable_flag==='init'){borderStyle=REDIPS.drag.border;cursor='move';enabled=true;}
else{borderStyle=REDIPS.drag.border_disabled;cursor='auto';enabled=false;}
...
for(i=0,j=0;i<divs.length;i++){if(regex_drag.test(divs[i].className)){if(enable_flag==='init'||divs[i].redips===undefined){divs[i].redips={};divs[i].redips.container=div_drag;}
...
register_events(divs[i],enabled);divs[i].style.borderStyle=borderStyle;divs[i].style.cursor=cursor;divs[i].redips.enabled=enabled;}
...
else{for(i=0,j=0;i<cn;i++){if(from.childNodes[j].nodeType===1&&from.childNodes[j].nodeName==='DIV'){div=from.childNodes[j];to.appendChild(div);register_events(div);}
...
else{relocate(c1,c2);}}};move_object=function(ip){var p={'direction':1},x1,y1,w1,h1,x2,y2,w2,h2,row,col,dx,dy,pos,i,target;p.callback=ip.callback;p.overwrite=ip.overwrite;if(typeof(ip.id)==='string'){p.obj=p.obj_old=document.getElementById(ip.id);}
...
else if(p.obj&&p.obj.className.indexOf('row')>-1){p.mode='row';p.obj=p.obj_old=obj_old=find_parent('TR',p.obj);p.obj=row_clone(p.obj_old);}
else{p.mode='cell';}
if(typeof(p.obj)!=='object'||p.obj===null){return;}
p.obj.style.zIndex=999;if(div_drag!==p.obj.redips.container){div_drag=p.obj.redips.container;init_tables();}
pos=box_offset(p.obj);w1=pos[1]-pos[3];h1=pos[2]-pos[0];x1=pos[3];y1=pos[0];if(ip.clone===true&&p.mode==='cell'){p.obj=clone_div(p.obj,true);REDIPS.drag.myhandler_cloned(p.obj);}
...
else{reset_styles(p.obj);p.obj.redips.animated=false;if(p.mode==='cell'){if(p.overwrite===true){empty_cell(p.target_cell);}
p.target_cell.appendChild(p.obj);register_events(p.obj);}
else{row_drop(get_table_index(p.target[0]),p.target[1],p.obj);}
if(typeof(p.callback)==='function'){p.callback(p.obj);}}};get_position=function(ip){var toi,toi_source,ci,ri,ti,el,tbl,arr=[];ci=ri=ti=-1;if(ip===undefined){if(table<tables.length){toi=tables[table].redips.idx;}
...
if(el){if(el.nodeName!=='TD'){el=find_parent('TD',el);}
if(el&&el.nodeName==='TD'){ci=el.cellIndex;ri=el.parentNode.rowIndex;tbl=find_parent('TABLE',el);ti=tbl.redips.idx;}}
arr=[ti,ri,ci];}
