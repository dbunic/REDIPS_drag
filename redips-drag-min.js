...
return false;}};return false;};handler_ondblclick=function(e){REDIPS.drag.myhandler_dblclicked();};table_top=function(obj){var e,idx,tmp;e=obj.parentNode;while(e&&e.nodeName!=='TABLE'){e=e.parentNode;}
...
div1=row_obj.getElementsByTagName('div');div2=table_mini.getElementsByTagName('div');for(i=0;i<div2.length;i++){div2[i].redips_enabled=div1[i].redips_enabled;div2[i].redips_container=div1[i].redips_container;if(div1[i].redips_enabled){div2[i].onmousedown=handler_onmousedown;div2[i].ondblclick=handler_ondblclick;}}
...
REDIPS.drag.myhandler_undeleted();}};enable_drag=function(enable_flag,div_id){var i,j,k,divs=[],tbls=[],borderStyle,cursor,overflow,autoscroll,enabled,cb,handler1,handler2,position,regex_drag=/\bdrag\b/i,regex_noautoscroll=/\bnoautoscroll\b/i;if(enable_flag===true||enable_flag==='init'){handler1=handler_onmousedown;handler2=handler_ondblclick;borderStyle=REDIPS.drag.border;cursor='move';enabled=true;}
else{handler1=null;handler2=null;borderStyle=REDIPS.drag.border_disabled;cursor='auto';enabled=false;}
