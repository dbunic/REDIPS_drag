...
return el;}};row_drop=function(r_table,r_row){var tr=obj.getElementsByTagName('tr')[0],ts=tables[r_table].rows[0].parentNode;tables[table_source].deleteRow(row_source);if(r_row<tables[r_table].rows.length){ts.insertBefore(tr,tables[r_table].rows[r_row]);}
else{ts.appendChild(tr);}
obj.parentNode.removeChild(obj);};handler_onmouseup=function(e){var evt=e||window.event,target_table,r_table,r_row,mt_tr,X,Y,i,target_elements,target_elements_length;X=evt.clientX;Y=evt.clientY;edge.flag.x=edge.flag.y=0;if(obj.releaseCapture){obj.releaseCapture();}
...
pos=box_offset(p.obj);w1=pos[1]-pos[3];h1=pos[2]-pos[0];x1=pos[3];y1=pos[0];if(target===undefined){target=get_position();}
...
else{p.obj.style.zIndex=-1;p.obj.style.position='static';p.target_cell.appendChild(p.obj);}};get_position=function(ip){var toi,toi_source,ci,ri,ti,el,tbl,arr=[];if(ip===undefined){if(table<tables.length){toi=tables[table].redips_idx;}
