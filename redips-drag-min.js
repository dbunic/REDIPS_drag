...
group_idx++;sort_idx++;}}};handler_onmousedown=function(e){var evt=e||window.event,offset,mouseButton,position,X,Y;evt.cancelBubble=true;if(evt.stopPropagation){evt.stopPropagation();}
if(evt.which){mouseButton=evt.which;}
else{mouseButton=evt.button;}
if(mouseButton!==1||elementControl(evt)){return true;}
X=pointer.x=evt.clientX;Y=pointer.y=evt.clientY;REDIPS.drag.obj_old=obj_old=obj||this;REDIPS.drag.obj=obj=this;table_top(obj);if(div_drag!==obj.redips.container){div_drag=obj.redips.container;init_tables();}
...
moved_flag=cloned_flag=0;REDIPS.event.add(document,'mousemove',handler_onmousemove);REDIPS.event.add(document,'mouseup',handler_onmouseup);if(obj.setCapture){obj.setCapture();}
...
tables.sort(function(a,b){return b.redips.sort-a.redips.sort;});sort_idx++;};row_clone=function(el){var table_mini,offset,row_obj,last_idx,last_row=true,cr,id,i,j;if(el.nodeName==='DIV'){id=el.id;el=find_parent('TR',el);if(el.redips===undefined){el.redips={};}
el.redips.dragrow_id=id;return el;}
else{row_obj=el;if(row_obj.redips===undefined){row_obj.redips={};}
el=find_parent('TABLE',el);table_mini=el.cloneNode(true);last_idx=table_mini.rows.length-1;for(i=last_idx;i>=0;i--){if(i!==row_obj.rowIndex){if(last_row===true){cr=table_mini.rows[i];for(j=0;j<cr.cells.length;j++){if(cr.cells[j].className.indexOf('rowhandler')>-1){last_row=false;}}}
table_mini.deleteRow(i);}}
row_obj.redips.last_row=last_row;table_mini.redips={};table_mini.redips.container=el.redips.container;table_mini.redips.source_row=row_obj;if(row_obj.redips!==undefined){table_mini.redips.dragrow_id=row_obj.redips.dragrow_id;}
form_elements(row_obj,table_mini.rows[0]);copy_properties(row_obj,table_mini.rows[0]);document.getElementById('redips_clone').appendChild(table_mini);offset=box_offset(row_obj,'fixed');table_mini.style.position='fixed';table_mini.style.top=offset[0]+"px";table_mini.style.left=offset[3]+"px";table_mini.style.width=(offset[1]-offset[3])+"px";return table_mini;}};row_drop=function(r_table,r_row,table_mini){var tbl=tables[r_table],ts=tbl.rows[0].parentNode,animated=false,tr,rp,src,rowIndex;if(table_mini===undefined){table_mini=obj;}
else{animated=true;}
src=table_mini.redips.source_row;rowIndex=src.rowIndex;src=find_parent('TABLE',src);if(!animated&&obj_old.redips.last_row){row_opacity(obj_old,'empty','White');}
else{src.deleteRow(rowIndex);}
tr=table_mini.getElementsByTagName('tr')[0];if(r_row<tbl.rows.length){ts.insertBefore(tr,tbl.rows[r_row]);rp=tbl.rows[r_row+1].redips;if(rp&&rp.last_row){ts.deleteRow(r_row+1);}}
else{ts.appendChild(tr);}
delete tr.redips.last_row;if(tr.getElementsByTagName('table').length>0){init_tables();}
...
obj.parentNode.removeChild(obj);delete obj_old.redips.last_row;REDIPS.drag.myhandler_row_dropped_source(target_cell);}
...
else if(cloned_flag===1&&REDIPS.drag.delete_cloned===true&&(X<target_table.redips.offset[3]||X>target_table.redips.offset[1]||Y<target_table.redips.offset[0]||Y>target_table.redips.offset[2])){obj.parentNode.removeChild(obj);cloned_id[obj_old.id]-=1;REDIPS.drag.myhandler_notcloned();}
...
REDIPS.drag.myhandler_dropped_before(target_cell);target_cell.appendChild(obj);if(target_elements_length){REDIPS.drag.myhandler_switched();REDIPS.drag.myhandler_dropped(target_cell);if(cloned_flag===1){REDIPS.drag.myhandler_cloned_dropped(target_cell);clone_limit();}}
else{REDIPS.drag.myhandler_dropped(target_cell);if(cloned_flag===1){REDIPS.drag.myhandler_cloned_dropped(target_cell);clone_limit();}}}
...
REDIPS.drag.myhandler_dropped_before(target_cell);target_cell.appendChild(obj);REDIPS.drag.myhandler_dropped(target_cell);if(cloned_flag===1){REDIPS.drag.myhandler_cloned_dropped(target_cell);clone_limit();}}
else{REDIPS.drag.myhandler_dropped_before(target_cell);target_cell.appendChild(obj);REDIPS.drag.myhandler_dropped(target_cell);if(cloned_flag===1){REDIPS.drag.myhandler_cloned_dropped(target_cell);clone_limit();}}
if(mode==='cell'&&obj.getElementsByTagName('table').length>0){init_tables();}
calculate_cells();}
table_old=row_old=cell_old=null;};handler_onmousemove=function(e){var evt=e||window.event,bound=REDIPS.drag.bound,sca,X,Y,i,scrollPosition;X=pointer.x=evt.clientX;Y=pointer.y=evt.clientY;if(moved_flag===0){if(obj.className.indexOf('clone')>-1||(REDIPS.drag.clone_shiftKey===true&&evt.shiftKey)){REDIPS.drag.obj_old=obj_old=obj;REDIPS.drag.obj=obj=clone_div(obj,true);cloned_flag=1;REDIPS.drag.myhandler_cloned();set_position();}
...
if(cell_current.parentNode.redips&&cell_current.parentNode.redips.last_row){previous();break;}}
...
div_cloned.id=div.id+'c'+cloned_id[div.id];cloned_id[div.id]+=1;copy_properties(div,div_cloned);return(div_cloned);};copy_properties=function(src,cln){var copy=[],childs;copy[0]=function(e1,e2){if(e1.redips){e2.redips={};e2.redips.enabled=e1.redips.enabled;e2.redips.container=e1.redips.container;if(e1.redips.enabled){e2.onmousedown=handler_onmousedown;e2.ondblclick=handler_ondblclick;}}};copy[1]=function(e1,e2){if(e1.redips){e2.redips={};e2.redips.dragrow_id=e1.redips.dragrow_id;e2.redips.last_row=e1.redips.last_row;}};childs=function(e){var el1,el2,i,tn=['DIV','TR'];el1=src.getElementsByTagName(tn[e]);el2=cln.getElementsByTagName(tn[e]);for(i=0;i<el2.length;i++){copy[e](el1[i],el2[i]);}};if(src.nodeName==='DIV'){copy[0](src,cln);}
...
divs[i].onmousedown=handler1;divs[i].ondblclick=handler2;divs[i].style.borderStyle=borderStyle;divs[i].style.cursor=cursor;divs[i].redips.enabled=enabled;}
...
j++;}}}};enable_table=function(enable_flag,cname){var i;for(i=0;i<tables.length;i++){if(tables[i].className.indexOf(cname)>-1){tables[i].redips.enabled=enable_flag;}}};get_style=function(el,style_name){var val;if(el&&el.currentStyle){val=el.currentStyle[style_name];}
...
return el;};save_content=function(tbl){var query='',tbl_start,tbl_end,tbl_rows,cells,tbl_cell,t,r,c,d;tables.sort(function(a,b){return a.redips.idx-b.redips.idx;});if(tbl===undefined){tbl_start=0;tbl_end=tables.length-1;}
...
query=query.substring(0,query.length-1);tables.sort(function(a,b){return b.redips.sort-a.redips.sort;});return query;};relocate=function(from,to){var i,childnodes_length;if(from===to){return;}
...
if(typeof(p.callback)==='function'){p.callback();}}};get_position=function(ip){var toi,toi_source,ci,ri,ti,el,tbl,arr=[];if(ip===undefined){if(table<tables.length){toi=tables[table].redips.idx;}
...
return i;};row_opacity=function(el,opacity,color){var td,i,j;if(typeof(el)==='string'){el=document.getElementById(el);el=find_parent('TABLE',el);}
