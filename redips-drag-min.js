...
calculate_cells();}
table_old=row_old=cell_old=null;};element_drop=function(){var drop=REDIPS.drag.myhandler_dropped_before(target_cell);if(drop!==false){target_cell.appendChild(obj);REDIPS.drag.myhandler_dropped(target_cell);if(cloned_flag===1){REDIPS.drag.myhandler_cloned_dropped(target_cell);clone_limit();}}
else if(cloned_flag===1){obj.parentNode.removeChild(obj);}};handler_onmousemove=function(e){var evt=e||window.event,bound=REDIPS.drag.bound,sca,X,Y,i,scrollPosition;X=pointer.x=evt.clientX;Y=pointer.y=evt.clientY;if(moved_flag===0){if(obj.className.indexOf('clone')>-1||(REDIPS.drag.clone_shiftKey===true&&evt.shiftKey)){REDIPS.drag.obj_old=obj_old=obj;REDIPS.drag.obj=obj=clone_div(obj,true);cloned_flag=1;REDIPS.drag.myhandler_cloned();set_position();}
