...
REDIPS.event.add(window,'scroll',calculate_cells);};init_tables=function(){var i,j,element,level,group_idx,tables_nodeList,nested_tables,td,rowspan;tables_nodeList=div_drag.getElementsByTagName('table');for(i=0;i<tables_nodeList.length;i++){element=tables_nodeList[i].parentNode;level=0;do{if(element.nodeName==='TD'){level++;element.table=true;}
element=element.parentNode;}while(element&&element!==div_drag);tables[i]=tables_nodeList[i];tables[i].redips_nestedLevel=level;tables[i].redips_idx=i;td=tables[i].getElementsByTagName('td');for(j=0,rowspan=false;j<td.length;j++){if(td[j].rowSpan>1){rowspan=true;break;}}
...
group_idx++;sort_idx++;}}};img_onmousemove=function(){return false;};handler_onmousedown=function(e){var evt=e||window.event,offset,mouseButton,position,X,Y;X=pointer.x=evt.clientX;Y=pointer.y=evt.clientY;if(elementControl(evt)){return true;}
...
tables.sort(function(a,b){return b.redips_sort-a.redips_sort;});sort_idx++;};row_clone=function(el){var table_mini,offset,row_obj,row_index,div1,div2,row_last,i;if(el.tagName==='TR'){row_obj=el;while(el&&el.nodeName!=='TABLE'){el=el.parentNode;}
...
childnodes_length=from.childNodes.length;for(i=0;i<childnodes_length;i++){to.appendChild(from.childNodes[0]);}};move_object=function(obj_id,target){var p={'direction':1},x1,y1,w1,h1,x2,y2,w2,h2,row,col,dx,dy,pos,i;p.obj=document.getElementById(obj_id);p.obj.style.zIndex=999;if(div_drag!==p.obj.redips_container){div_drag=p.obj.redips_container;init_tables();}
pos=box_offset(p.obj);w1=pos[1]-pos[3];h1=pos[2]-pos[0];x1=pos[3];y1=pos[0];if(target===undefined){target=location();}
for(i=0;i<tables.length;i++){if(tables[i].redips_idx===target[0]){break;}}
row=target[1];col=target[2];p.target_cell=tables[i].rows[row].cells[col];pos=box_offset(p.target_cell);w2=pos[1]-pos[3];h2=pos[2]-pos[0];x2=pos[3]+(w2-w1)/2;y2=pos[0]+(h2-h1)/2;dx=x2-x1;dy=y2-y1;p.obj.style.position='fixed';if(Math.abs(dx)>Math.abs(dy)){p.type='horizontal';p.m=dy/dx;p.b=y1-p.m*x1;p.k1=(x1+x2)/(x1-x2);p.k2=2/(x1-x2);if(x1>x2){p.direction=-1;}
i=x1;p.last=x2;}
else{p.type='vertical';p.m=dx/dy;p.b=x1-p.m*y1;p.k1=(y1+y2)/(y1-y2);p.k2=2/(y1-y2);if(y1>y2){p.direction=-1;}
i=y1;p.last=y2;}
animation(i,p);};animation=function(i,p){var k=(p.k1-p.k2*i)*(p.k1-p.k2*i),f;i=i+animation_step*(4-k*3)*p.direction;f=p.m*i+p.b;if(p.type==='horizontal'){p.obj.style.left=i+'px';p.obj.style.top=f+'px';}
else{p.obj.style.left=f+'px';p.obj.style.top=i+'px';}
if((i<p.last&&p.direction>0)||((i>p.last)&&p.direction<0)){setTimeout(function(){animation(i,p);},animation_pause*k);}
else{p.obj.style.zIndex=-1;p.obj.style.position='static';p.target_cell.appendChild(p.obj);}};location=function(tc){var toi,toi_source,ci,ri,ti,tbl,arr=[];if(tc===undefined){if(table<tables.length){toi=tables[table].redips_idx;}
